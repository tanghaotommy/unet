...
class FcnHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.foreground = nn.Conv2d(channels, 1,   kernel_size=3, padding=1)
        self.border     = nn.Conv2d(channels, 1,   kernel_size=3, padding=1)
    ...
    def forward(self, f):
        f = self.up(f)
        f = F.relu(self.conv(f), inplace=True)
        f = F.dropout(f, p=0.5,training=self.training)
        foregrounds  = self.foreground(f)
        borders = self.border(f)

        return foregrounds, borders
...
class UNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        self.foregrounds, self.borders = data_parallel(self.fcn_head, features)




    def criterion(self, truth_foregrounds, truth_borders, foreground_weights):
        cfg  = self.cfg
        #B,C,H,W = truth_foregrounds.size()

        foregrounds = self.foregrounds.view(-1)
        truth_foregrounds = truth_foregrounds.view(-1)
        foreground_weights = foreground_weights.view(-1)
        self.foreground_loss = F.binary_cross_entropy_with_logits(foregrounds, truth_foregrounds, 
            weight=foreground_weights, size_average=True)

        borders = self.borders.view(-1)
        truth_borders = truth_borders.view(-1)
        self.border_loss = F.binary_cross_entropy_with_logits(borders, truth_borders, size_average=True)



        # distances = self.distances.view(-1)
        # truth_distances = truth_distances.view(-1)
        # self.distance_loss = F.mse_loss(distances,truth_distances, size_average=True)

        # l = (distances-truth_distances)**2
        # l = l.sum()/(B*H*W) *0.5
        # self.distance_loss = l

        self.loss = self.foreground_loss + self.border_loss
        #self.loss = self.border_loss
...
def run_check_u_net():
    ...
    print('distances  ',net.distances.size())
    print('')
